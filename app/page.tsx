 ...
 * - Kids count + age; US healthcare tri-pill + override
 * - Savings slider %; Student loan slider; Debt slider
 * - All “How you stay okay” sliders wide range
 * Data:
 * - Session-based ingest (single upsert) + A/B email CTA variant
 */

// ---------- Data capture config ----------
const DATA_MODE: 'api' | 'supabase' = 'api';
...
const URBANICITY: Record<Urbanicity, { label: string; rentMul: number; commuteMul: number }> = {
  inner:    { label: 'Inner-city core', rentMul: 1.50, commuteMul: 1.10 },
  city:     { label: 'City / metro',    rentMul: 1.20, commuteMul: 1.00 },
  suburban: { label: 'Suburban',        rentMul: 1.00, commuteMul: 1.10 },
  rural:    { label: 'Rural / small town', rentMul: 0.85, commuteMul: 1.20 },
};

const COMMUTE_CTX: Record<CommuteContext, { label: string; commuteMul: number }> = {
  transitEfficient: { label: 'Transit-efficient city', commuteMul: 1.00 },
  mixed:            { label: 'Mixed options',          commuteMul: 1.10 },
  carDependent:     { label: 'Car-dependent city',     commuteMul: 1.30 },
};
...
function childCostPreset(regionId: RegionId, count: number, age: ChildAge) {
  if (count<=0) return 0;
  const base = regionId==='UK'? 400 : regionId==='EU'? 300 : regionId==='US'? 500 : 280; // per child baseline
  const ageMul = age==='nursery' ? 1.5 : 1.0; // nursery more expensive
  const kidsAdj = count===1?1: count===2?1.9: 2.6; // diminishing after 2+
...
function computeHealthcareUS(plan: HealthPlanUS){ return plan==='employer'?200: plan==='market'?450: 150; }
...
function computeSavingsFromRate(netMonthly: number, ratePct: number) { return Math.round(netMonthly * Math.max(0, Math.min(20, ratePct)) / 100); }
...
const Money: React.FC<{value: number; currency: string}> = ({value, currency}) => (<span className="tabular-nums font-semibold">{currency}{Math.max(0,value).toLocaleString()}</span>);
  ...
  return (
    <div className="space-y-2">
      <div className="w-full h-6 rounded-lg overflow-hidden bg-zinc-800">
        ...
        {slices.map(s => (<div key={s.label} className={`h-6 ${s.color} float-left`} style={{ width: `${(Math.max(0,s.value)/safeNet)*100}%`}} title={`${s.label} ${currency}${Math.max(0,s.value).toLocaleString()}`} />))}
      </div>
      <div className="flex flex-wrap gap-3 text-xs text-zinc-300">
        {slices.map(s => (<div key={s.label} className="flex items-center gap-1"><span className={`inline-block w-3 h-3 rounded ${s.color}`} />{s.label} {currency}{Math.max(0,s.value).toLocaleString()}</div>))}
...
export default function Page(){
  ...
  const [takeHome, setTakeHome] = useState<number>(2200); // net unless isGross=true
  ...
  useEffect(()=>{ if(!housingTouched){ setHousing(Math.round(suggestedHousing(region, household)*rentMul)); } },[region,household,urbanicity]); // eslint-disable-line
  ...
  const billsUtilities = useMemo(()=> billsIncluded? 0 : Math.round((region==='US'?180: region==='UK'?130: 120) * (urbanicity==='inner'?1.15:1)), [billsIncluded, region, urbanicity]);
  ...
  const maintenanceSum = useMemo(()=> driversSum + variableSum + billsUtilities + (transportMode==='remote'? wfhUtilities : 0), [driversSum, variableSum, billsUtilities, transportMode, wfhUtilities]);

  const leftover = useMemo(()=> Math.round(netMonthly - housing - commuteMonthly - maintenanceSum - dependentsMonthly - healthcareMonthly - debtMonthly - studentLoan - savingsMonthly), [netMonthly,housing,commuteMonthly,maintenanceSum,dependentsMonthly,healthcareMonthly, debtMonthly, studentLoan, savingsMonthly]);
  const effectivePerHour = useMemo(()=>{ const per=leftover/Math.max(1,hoursPerMonth); return Number.isFinite(per)? Math.round(per*100)/100 : 0; },[leftover,hoursPerMonth]);
  const maintenancePct = useMemo(()=> Math.max(0, Math.round((maintenanceSum/Math.max(1,netMonthly))*100)), [maintenanceSum, netMonthly]);

  // Baseline (PT + Typical drivers amounts), includes billsUtilities + variableSum
  ...
  const baselineLeftover = useMemo(()=> Math.round(netMonthly - housing - baselineCommute - baselineMaintenance - dependentsMonthly - healthcareMonthly - debtMonthly - studentLoan - savingsMonthly), [netMonthly,housing,baselineCommute, baselineMaintenance, dependentsMonthly, healthcareMonthly, debtMonthly, studentLoan, savingsMonthly]);
  const baselineFreedom = useMemo(()=>{ const per=baselineLeftover/Math.max(1,hoursPerMonth); return Number.isFinite(per)? Math.round(per*100)/100 : 0; },[baselineLeftover,hoursPerMonth]);

  const deltaRemote1d = useMemo(()=> Math.round(baselineCommute*(1/5)), [baselineCommute]);
  ...
  async function makeShareCard(){ if(!shareRef.current) return; const canvas=await html2canvas(shareRef.current,{backgroundColor:'#0a0a0a',scale:2}); setImageUrl(canvas.toDataURL('image/png')); }
  ...
  async function saveResult(opts:{force?:boolean}={})
  {
    ...
    try{
      ...
      const payload = {
        session_id:sessionId, land_source:landSource, ab_variant:abVariant, step_posted:step,
        city_name:cityName||null, region, urbanicity, commute_context:commuteCtx, rent_mul:rentMul, commute_mul:commuteMul,
        is_gross:isGross, take_home_input:takeHome, net_monthly:netMonthly,
        household, children_count:childrenCount, children_age:childrenAge,
        housing, bills_included:billsIncluded,
        commute_mode:transportMode, commute_monthly:commuteMonthly, wfh_utilities:wfhUtilities,
        hours_week:hoursWeek,
        drivers, spends,
        dependents_monthly:dependentsMonthly, healthcare_monthly:healthcareMonthly, us_health_plan:usHealthPlan, us_health_override:usHealthcareOverride,
        debt_monthly:debtMonthly, student_loan:studentLoan, savings_rate:savingsRate, savings_monthly:savingsMonthly,
        bills_utilities:billsUtilities,
        leftover, effective_per_hour:effectivePerHour, maintenance_pct:maintenancePct,
        baseline_commute:baselineCommute, baseline_leftover:baselineLeftover, baseline_freedom:baselineFreedom,
        delta_remote_1d:deltaRemote1d, delta_debt_refi:deltaDebt, delta_total:deltaTotal,
        email: email||null
      };
      if (DATA_MODE === 'api') {
        ...
        <div className="space-y-6 mt-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={region} onChange={e=>setRegion(e.target.value as RegionId)} className="w-full mt-2 rounded-lg border p-2 bg-white dark:bg-zinc-900">{regions.map(r=>(<option key={r.id} value={r.id}>{r.label}</option>))}</select>
            </div>
            <div>
              <label className="text-sm">Area type</label>
              <select value={urbanicity} onChange={e=>setUrbanicity(e.target.value as Urbanicity)} className="w-full mt-2 rounded-lg border p-2 bg-white dark:bg-zinc-900">{Object.entries(URBANICITY).map(([k,v])=>(<option key={k} value={k}>{v.label}</option>))}</select>
            </div>
            <div>
              <label className="text-sm">Commute context</label>
              <select value={commuteCtx} onChange={e=>setCommuteCtx(e.target.value as CommuteContext)} className="w-full mt-2 rounded-lg border p-2 bg-white dark:bg-zinc-900">{Object.entries(COMMUTE_CTX).map(([k,v])=>(<option key={k} value={k}>{v.label}</option>))}</select>
      ...
      <CardBody>
        ...
        <div className="grid md:grid-cols-2 gap-5 mt-6">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-1">Typical for {regions.find(r=>r.id===region)?.label} × {URBANICITY[urbanicity].label}: {currency}{Math.round(suggestedHousing(region, household)*rentMul).toLocaleString()} · <button type="button" className="underline" onClick={()=>{setHousing(Math.round(suggestedHousing(region, household)*rentMul)); setHousingTouched(true);}}>Use this</button></p>
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap mt-2">
              {(['solo','partner','partnerKids','singleParent','share','family'] as Household[]).map(h=>(
                <button key={h} onClick={()=>{setHousehold(h); if(h==='partner'||h==='solo'||h==='share'||h==='family'){ setChildrenCount(0);} }} className={`px-3 py-2 rounded-full border text-sm ${household===h?'bg-zinc-900 text-white border-zinc-900':'border-zinc-300'}`}>{h==='solo'?'Solo':h==='partner'?'Partner':h==='partnerKids'?'Partner + kids':h==='singleParent'?'Single parent':h==='share'?'House share':'Back home'}</button>
              ))}
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap mt-2">
                {(['employer','market','none'] as HealthPlanUS[]).map(p=>(
                  <button key={p} onClick={()=>setUsHealthPlan(p)} className={`px-3 py-2 rounded-full border text-sm ${usHealthPlan===p?'bg-zinc-900 text-white border-zinc-900':'border-zinc-300'}`}>{p==='employer'?'Employer plan':p==='market'?'Marketplace':'Uninsured'}</button>
                ))}
              </div>
              <div className="mt-2">
  ...
  const simLeftover = Math.round(simNet - simHousing - simCommute - maintenanceSum - dependentsMonthly - healthcareMonthly - debtMonthly - studentLoan - savingsMonthly);
      ...
      <CardBody>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mt-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
            <input placeholder={abVariant==='A'? 'you@email.com — get the 1-page plan' : 'Email to unblur your fixes'} value={email} onChange={e=>setEmail(e.target.value)} className="w-72 px-4 py-2 rounded-lg border bg-white dark:bg-zinc-900" />
            <button onClick={saveEmail} className={`px-4 py-2 rounded-lg font-medium transition ${emailSaved ? 'border border-emerald-600 text-emerald-600 bg-white dark:bg-zinc-900' : 'text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700'}`}>{emailSaved ? (abVariant==='A'?'Email saved':'Saved') : (abVariant==='A'?'Email me the 1-page plan':'Unlock my plan')}</button>
          </div>
        </div>
        ...
        <div className="mt-5 grid md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="text-3xl font-bold mt-1">Baseline (PT + Typical): {currency}{Math.max(0,baselineLeftover).toLocaleString()} kept over {hoursPerMonth}h</div>
                ...
                {netMonthly>0 && (
                  <div className="text-sm text-zinc-400 mt-2">Out of every {currency}1 you earn, {currency}{(1 - Math.max(0,baselineLeftover)/netMonthly).toFixed(2)} goes to staying employable and functional.</div>
                )}
          ...
          <div className="space-y-5">
            ...
            <Card><CardBody>
              ...
              <div className="mt-3 text-xs text-zinc-500">Simulated: Keep <span className="font-semibold">{currency}{Math.max(0, simLeftover).toLocaleString()}</span> ({currency}{simFreedom.toFixed(2)}/hr)</div>
