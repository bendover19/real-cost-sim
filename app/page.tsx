...
/**
 * Real Cost Simulator — single-file page component
 * - Removed percentiles/benchmarks + any Supabase references
 * - Stable session_id + client-side post guard to avoid duplicate rows
 * - Posts to /api/ingest only (server should upsert by session_id)
 */
...
> = {
  belonging: {
    emoji: "🫶",
    title: "Belonging",
    sub: "Not being the ghost at work or with friends",
    color: "rose",
  },
  identity: {
    emoji: "👔",
    title: "Identity",
    sub: "Looking like you belong where you work",
    color: "violet",
  },
  timeTrade: {
    emoji: "⏱️",
    title: "Time trade",
    sub: "When tired, you buy time (delivery, taxis)",
    color: "amber",
  },
  comfort: {
    emoji: "🌙",
    title: "Comfort",
    sub: "Switching off after rough days",
    color: "sky",
  },
  connection: {
    emoji: "💘",
    title: "Connection",
    sub: "Dating / keeping relationships alive",
    color: "fuchsia",
  },
  treats: {
    emoji: "🎁",
    title: "Small treats",
    sub: "Subscriptions and little upgrades",
    color: "emerald",
  },
  moneyPressure: {
    emoji: "💸",
    title: "Money pressure",
    sub: "BNPL/overdraft to smooth the month",
    color: "orange",
  },
};
...
> = {
  pet: {
    label: "🐾 Pet costs (food, insurance, sitter/boarding, horse, etc.)",
    min: 0,
    max: 2000,
    step: 10,
  },
  therapy: {
    label: "🧠 Therapy / coaching / counselling",
    min: 0,
    max: 2000,
    step: 10,
  },
  supportOthers: {
    label: "🤝 Support others (family remit, gifts, obligations)",
    min: 0,
    max: 3000,
    step: 10,
  },
  health: {
    label: "💊 Health costs (meds, dental, vision, non-insurance)",
    min: 0,
    max: 2000,
    step: 10,
  },
...
const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div
    className={`rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur ${
      className || ""
    }`}
  >
    {children}
...
function BarChart({
  ...
  const used = Math.min(
    housing + commute + dependents + healthcare + debt + savings + maintenance,
    safeNet
  );
...
export default function Page() {
  ...
  // Stable session id (prevents empty session on first post)
  const [sessionId] = useState<string>(() => {
    try {
      ...
      if (!existing) localStorage.setItem("rcs_session_id", id);
      return id;
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (existing) {
        setAbVariant(existing[1] as "A" | "B");
      } else {
  ...
  const [transportMode, setTransportMode] =
    useState<"pt" | "drive" | "remote" | "walk">("pt");
  ...
  // Drivers numeric
  const [drivers, setDrivers] = useState<Record<DriverKey, number>>({
    ...DRIVER_TYPICAL,
  });
  ...
  const isSavingRef = useRef(false); // prevents overlapping posts
  ...
  const variableSum = useMemo(
    () => spends.pet + spends.therapy + spends.supportOthers + spends.health,
    [spends]
  );
  const billsUtilities = useMemo(
    () =>
      billsIncluded
        ? 0
        : Math.round(
            (region === "US" ? 180 : region === "UK" ? 130 : 120) *
              (urbanicity === "inner" ? 1.15 : 1)
          ),
  ...
  const maintenanceSum = useMemo(
    () =>
      ...
      variableSum +
      billsUtilities +
      (transportMode === "remote" ? wfhUtilities : 0),
  ...
  async function saveResult(opts: { force?: boolean } = {}) {
    ...
    try {
      const payload = {
        ...
        delta_total: Math.max(
          0,
          Math.round(baselineCommute * (1 / 5)) + Math.round((debtMonthly + studentLoan) * 0.15)
        ),
        email: email || null,
      };
  ...
  function saveEmail() {
    if (!email) return;
    setEmailSaved(true);
    // force a post; server should upsert by session_id so we don't create a second row
    saveResult({ force: true });
  }

  useEffect(() => {
    // single auto-post when user reaches reveal
    if (step >= 3 && sessionId && !postedOnce) saveResult({ force: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [step, sessionId, postedOnce]);

  // --- Validation helpers ---
  const takeHomeWeird =
    (!isGross && takeHome > 20000) || (isGross && takeHome > 30000);
  const housingWeird =
    housingTouched &&
    housing < 300 &&
    !((["family", "share"] as Household[]).includes(household));
      ...
      <CardBody>
        ...
        <div className="space-y-6 mt-6">
          ...
          <p className="text-zinc-700 dark:text-zinc-300">
            See your <span className="font-semibold">hour of freedom</span> after rent,
            commute, and the real cost of staying employable.
          </p>
          <p className="text-zinc-700 dark:text-zinc-300 font-medium">
            In <span className="font-bold text-lg text-zinc-900 dark:text-white">one minute</span>{" "}
            you’ll get the truth, then what to change.
          </p>
      ...
      <CardBody>
        ...
        <div className="grid md:grid-cols-2 gap-5 mt-6">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-1">
              Typical for {regions.find((r) => r.id === region)?.label} ×{" "}
              {URBANICITY[urbanicity].label}: {currency}
            ...
            {transportMode === "remote" && (
              <div className="mt-2">
                ...
                <input
                  ...
                  step={5}
                  value={wfhUtilities}
                  onChange={(e) => setWfhUtilities(Number(e.target.value))}
                  className="w-full"
                />
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap mt-2">
              {(
                ["solo", "partner", "partnerKids", "singleParent", "share", "family"] as Household[]
              ).map((h) => (
                <button
                  key={h}
                  onClick={() => {
          ...
          <div>
            ...
            <input
              ...
              step={10}
              value={debtMonthly}
              onChange={(e) => setDebtMonthly(Number(e.target.value))}
              className="w-full mt-3"
            />
            <div className="text-xs text-zinc-500 mt-1">
          ...
          <div>
            ...
            <div className="text-xs text-zinc-500 mt-1">
              {savingsRate}% →{" "}
              <Money value={computeSavingsFromRate(netMonthly, savingsRate)} currency={currency} /> / month
            </div>
          </div>
            ...
            <div>
              ...
              <div className="mt-2">
                ...
                <input
                  ...
                  onChange={(e) => setUsHealthcareOverride(Number(e.target.value))}
                  className="w-full"
                />
                <div className="text-[11px] text-zinc-500">
                  If 0, we use a typical value for your plan.
                </div>
              </div>
            </div>
          )}
            ...
            return (
              <div key={k} className="border rounded-2xl p-4">
                ...
                <input
                  ...
                  step={lim.step}
                  value={spends[k]}
                  onChange={(e) => setSpends((s) => ({ ...s, [k]: Number(e.target.value) }))}
                  className="w-full"
                />
                <div className="text-xs text-zinc-500 mt-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mt-4">
          <div>
            <div className="flex items-center gap-2 text-xs text-zinc-600">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border">
                {badgeLeft}
              </span>
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 border">
                PT baseline
            ...
            <p className="text-sm text-zinc-500 max-w-md">
              Standardised baseline shows Public Transport + Typical behaviours for your area. Your
              choices change the deltas.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
            <input
              placeholder={
                abVariant === "A" ? "you@email.com — get the 1-page plan" : "Email to unblur your fixes"
              }
              value={email}
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="text-lg mt-1">
                  That's {currency}
                  {baselineFreedom.toFixed(2)} per hour of freedom
                ...
                {netMonthly > 0 && (
                  <div className="text-3xl font-bold mt-1">
                    ...
                    {(1 - Math.max(0, baselineLeftover) / netMonthly).toFixed(2)} goes to staying
                    employable and functional.
                  </div>
                )}

                {/* No percentiles/benchmarks (removed) */}
                ...
                <div className="absolute inset-0 grid place-items-center">
                  <div className="backdrop-blur-sm bg-zinc-900/70 border border-zinc-700 rounded-xl p-5 text-center max-w-sm mx-4">
                    ...
                    <div className="text-sm text-zinc-300 mt-1">
                      Enter your email to reveal the full breakdown and fixes.
                    </div>
              ...
              <CardBody>
                ...
                <div className="mt-2 space-y-3 text-sm">
                  <div>
                    ...
                    <input
                      ...
                      step={1}
                      value={simRemoteDays}
                      onChange={(e) => setSimRemoteDays(Number(e.target.value))}
                      className="w-full"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      step={50}
                      value={simRentDelta}
                      onChange={(e) => setSimRentDelta(Number(e.target.value))}
                      className="w-full"
                    />
                  </div>
                  <div>
                ...
                <div className="mt-3 text-xs text-zinc-500">
                  ...
                  <span className="font-semibold">
                    {currency}
                    {Math.max(0, simLeftover).toLocaleString()}
                  </span>{" "}
                  ({currency}
                  {simFreedom.toFixed(2)}/hr)
                </div>
                <div className="text-xs text-zinc-500">
                  Δ vs your baseline: {currency}
                  {(simLeftover - baselineLeftover).toLocaleString()}
                </div>
                {!emailSaved && (
                  <div className="text-[11px] text-zinc-500 mt-2">
                    Email your optimised plan & settings.
                  </div>
                )}
              </CardBody>
            </Card>
  ...
  return (
    <div
      className="min-h-screen text-zinc-900 dark:text-zinc-50 py-10
...
(function runSelfTests() {
  ...
  console.assert(
    typicalSum === 140 + 110 + 120 + 90 + 120 + 45 + 25,
    "Driver typical sum matches"
  );
  ...
  console.assert(
    childCostPreset("UK", 2, "nursery") > childCostPreset("UK", 1, "school"),
    "Nursery & more kids cost more"
  );
})();
